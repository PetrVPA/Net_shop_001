...
def test_download_file():
    raw_data = download_file("C:\\Users\\Nochtcha\\PycharmProjects\\Net_shop_001\\data\\products.json")
    assert raw_data == data_test


def test_create_category_from_json():
    answer = create_category_from_json(data_test)
    assert answer[0].products[0] == dict(name='Samsung Galaxy C23 Ultra', description='256GB, Серый цвет,'
                                         ' 200MP камера', price=180000.0, quantity=5)
    assert answer[0].products == [
      {'name': 'Samsung Galaxy C23 Ultra',
       'description': '256GB, Серый цвет, 200MP камера',
       'price': 180000.0, 'quantity': 5},
      {'name': 'Iphone 15', 'description': '512GB, Gray space', 'price': 210000.0, 'quantity': 8},
      {'name': 'Xiaomi Redmi Note 11', 'description': '1024GB, Синий', 'price': 31000.0, 'quantity': 14}]
    assert answer[1].products[0] == {'name': '55" QLED 4K', 'description': 'Фоновая подсветка',
